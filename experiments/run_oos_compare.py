"""
OOS 비교 실험: 2024~2025 vs 2025~2026
- LA=24h, DZ=1%, 월간 TTA
- Fear&Greed 1일 shift 적용 (미래 정보 유출 수정)
"""
import os, sys
...
exp_dir = create_experiment("oos_compare_fg_fixed", {
    "la": LA, "dz": DZ, "retrain_every": RETRAIN_EVERY,
    "fix": "fear_greed 1-day shift applied",
})
...
for pi, (oos_start, oos_end, label) in enumerate(periods):
    print(f"\n{'='*60}")
    print(f"  OOS: {label} | LA={LA}h DZ={DZ*100:.0f}% Retrain/{RETRAIN_EVERY}h")
    print(f"{'='*60}")
    ...
    print(f"\n  [{label}] Return={result['model_return_pct']:+.2f}% | "
    ...
    all_results.append({
        "period": label, "return": result["model_return_pct"],
    ...
    ax_compare.plot(dates, pv_norm, color=colors[pi], linewidth=2,
                    label=f"{label} Model ({result['model_return_pct']:+.2f}%)")

    # B&H 기준선
    bh_prices = oos_df["close"].values
    bh_norm = bh_prices / bh_prices[0]
    bh_ret = (bh_prices[-1] / bh_prices[0] - 1) * 100
    ax_compare.plot(oos_df["date"].values[:len(bh_norm)], bh_norm,
                    color=colors[pi], linewidth=1.5, linestyle="--", alpha=0.5,
                    label=f"{label} B&H ({bh_ret:+.2f}%)")
    ...
    st_list = [all_dates[t[0]] for t in trade_log if t[1] == "SELL"]
    ...
    ax.scatter(st_list, sp, marker="v", color="#E53935", s=100, zorder=5,
               label=f"Sell ({len(st_list)})", edgecolors="white", linewidths=0.5)
    for j in range(min(len(bt), len(st_list))):
        ax.axvspan(bt[j], st_list[j], alpha=0.06, color="#2196F3")
    for rt in retrain_points[1:]:
        ax.axvline(x=rt, color="#9C27B0", alpha=0.3, linestyle=":", linewidth=1)
    ax.set_title(
        f"[{label}] LA={LA}h DZ={DZ*100:.0f}%  |  "
    ...
    ax.plot(all_dates[:len(bh_norm2)], bh_norm2, color="#FF9800", linewidth=2, alpha=0.7,
    ...
    plt.tight_layout()
    path = os.path.join(exp_dir, f"trades_{label.replace('~','_')}.png")
...
ax_compare.set_title("OOS Compare: Fear&Greed 1-day shift fix | LA=24h DZ=1% + TTA",
...
path = os.path.join(exp_dir, "oos_comparison.png")
...
# 최종 요약
print(f"\n{'='*80}")
print(f"{'Period':>12} | {'Return':>10} | {'B&H':>8} | {'Excess':>8} | {'Sharpe':>7} | {'MDD':>8} | {'Trades':>6}")
print("-" * 80)
for r in all_results:
    excess = r["return"] - r["bh"]
    print(f"  {r['period']:>10} | {r['return']:>+9.2f}% | {r['bh']:>+7.2f}% | "
          f"{excess:>+7.2f}% | {r['sharpe']:>6.2f} | {r['mdd']:>7.2f}% | {r['trades']:>6}")
print(f"{'='*80}")
